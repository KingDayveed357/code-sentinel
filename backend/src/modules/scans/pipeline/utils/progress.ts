// src/modules/scans/pipeline/utils/progress.ts
...
export class ProgressTracker {
   ...
   */
  async emit(
    stage: keyof typeof this.stageProgressMap,
    message: string,
  ...
  async updateScannerProgress(
    ...
    const message = `Running ${currentScanner} (${completedCount}/${scannersRunning.length} completed)`;

    const event: ProgressEvent = {
        scanId: this.scanId,
        stage: 'scanner_start', // Use a valid stage
        progress_percent,
        current_scanner: currentScanner,
        message,
        timestamp: new Date().toISOString(),
