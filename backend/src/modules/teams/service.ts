...
export class TeamService {
  ...
  async createTeam(
    ...
    // Log team creation
    await this.logActivity(team.id, userId, "team.created", "team", team.id, {
      team_name: data.name,
    });
  ...
  > {
    ...
    const { data: members, error: membersError } = await this.fastify.supabase
      .from("team_members")
      .select(
        "role, team_id, teams!inner(id, name, slug, owner_id, plan, created_at, updated_at)"
      )
      .eq("user_id", userId)
      .eq("status", "active");
    ...
    const { data: memberCounts, error: countError } =
      await this.fastify.supabase
        .from("team_members")
        .select("team_id")
        .in("team_id", teamIds)
  ...
  /**
   * Update team name
   * Also updates the associated team workspace name to maintain sync
   * Workspace name format: "{teamName} Workspace"
   */
  async updateTeamName(
    ...
    if (team.owner_id !== updatedBy) {
      throw this.fastify.httpErrors.forbidden(
        "Only team owner can rename the team"
      );
    }
    ...
    // Update associated team workspace with new name format: "{teamName} Workspace"
    const newWorkspaceName = `${newName} Workspace`;
    try {
      const { error: workspaceError } = await this.fastify.supabase
        .from("workspaces")
        .update({
      ...
      if (workspaceError) {
        this.fastify.log.error(
          { error: workspaceError, teamId },
          "Failed to update team workspace name"
        );
        // Log but don't throw - team name was updated successfully
      }
    } catch (workspaceUpdateError) {
      this.fastify.log.error(
        { error: workspaceUpdateError, teamId },
        "Error updating team workspace name"
      );
      // Continue - don't block team update if workspace update fails
    }

    // Log team rename
    await this.logActivity(teamId, updatedBy, "team.renamed", "team", teamId, {
      old_name: team.name,
      new_name: newName,
  ...
  async deleteTeam(teamId: string, deletedBy: string): Promise<void> {
    ...
    await this.logActivity(teamId, deletedBy, "team.deleted", "team", teamId, {
      team_name: team.name,
    });
